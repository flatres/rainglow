  ...
  <!-- menuOpen = false; notificationsOpen = false -->
    ...
    <div class='no-shadow flex items-center justify-start' style='position: relative; z-index:997'>
      <q-btn
        ...
        :style='dimensions'
        >
        <q-avatar :style='!url && !forceProfileImgPath ? "border: 3px solid white" : ""' :size="widthPixels" class='text-white x-text-heavy' font-size='40%' :class='!url && !forceProfileImgPath ? "bg-gradient" : ""'>
          ...
          <q-menu
            ...
            style='height:334px; min-height:194px'
            >
            <q-list style="min-width: 265px; min-height:194px" class='q-py-sm q-px-md x-text-heavy x-text-md' v-if='!notificationsOpen'>
              ...
              <q-separator spaced/>
              <q-item class='q-px-xs' clickable @click.stop='menuOpen = false; $emit("trip")'>
                <q-item-section>Plan A Trip</q-item-section>
              </q-item>
              <q-separator spaced/>
              <q-item class='q-px-xs' clickable @click='$router.push({path: "/trips"})'>
              ...
              <q-separator spaced />
              <q-item @click='menuOpen = false; logoutUser()' class='q-px-xs' clickable>
      ...
      <q-badge v-if='hasUnread && hasMenu && !noBadge' class='z-max x-notifications-badge x-text-heavy text-white' color="accent" style='border: 1px solid white'>{{unreadCount}}</q-badge>
      <q-btn v-if='leader' class='bg-white x-crown flat z-max' filled icon='fas fa-crown fa-xs' round text-color='accent' size='sm'/>
    </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState(useUserStore, ['isLoggedIn', 'name', 'notifications', 'profile', 'profileUrl']),
    unreadNotifications () {
